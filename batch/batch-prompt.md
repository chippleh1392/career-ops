...
Turn “builder” into a professional signal, not a “hobby maker.” Framing changes; the facts stay the same.
...
2. **Plan to “sell senior without lying”:** specific phrasing, concrete wins, founder angle as advantage
...
Where `{company-slug}` is the company name in lowercase, no spaces, hyphens.
...
| `{{SUMMARY_TEXT}}` | Summary with keywords injected |
...
**IMPORTANT:** In the TSV, **status** comes **before** score (col 5 → status, col 6 → score). In `applications.md` the order is reversed (col 5 → score, col 6 → status). `merge-tracker.mjs` handles the conversion.
...
3. Put phone numbers in generated messages
...
7. When you generate English text (PDF summaries, bullets, STAR stories), use native tech English: short phrases, strong verbs, avoid unnecessary passive voice, avoid "in order to" and "utilized"
