...
- No critical info only in PDF headers/footers (ATS ignores them)
...
**NEVER add skills the candidate lacks. Only rephrase real experience with exact JD vocabulary.**
...
| `{{EXPERIENCE}}` | HTML per role with reordered bullets |
...
Update the tracker if the posting is already registered: change PDF from ❌ to ✅.
